...
func (pq *PictureQuery) QueryUser() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, true, picture.UserTable, picture.UserPrimaryKey...),
		)
...
func (pq *PictureQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Picture, error) {
	...
	if query := pq.withUser; query != nil {
		...
		query.Where(func(s *sql.Selector) {
			joinT := sql.Table(picture.UserTable)
			s.Join(joinT).On(s.C(user.FieldID), joinT.C(picture.UserPrimaryKey[0]))
			s.Where(sql.InValues(joinT.C(picture.UserPrimaryKey[1]), edgeids...))
			columns := s.SelectedColumns()
			s.Select(joinT.C(picture.UserPrimaryKey[1]))
